...
export async function parseWithGemini(query: string, availableFields: FieldMeta[], apiKey: string): Promise<Intent | null> {
    ...
    const prompt = `Classify the user query into JSON: { "chartType": "histogram"|"bar"|"line"|"pie"|"unknown", "xField": string, "yField": string, "valueField": string }. 
Query: "${query}"
...
Respond with JSON only, no markdown.`;
    ...
    try {
        ...
        const intent: Intent = { chartType: parsed.chartType || 'unknown' };
